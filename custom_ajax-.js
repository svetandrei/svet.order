...
BX.Sale.OrderAjaxComponent = {
    ...
    getData: function(action, actionData)
    {
        ...
        //data.order[this.params.ACTION_VARIABLE] = action;
        return data;
    },
        ...
        for (let p in this.result.GRID.ROWS) {
            ...
            itemNodeInnerInner.appendChild(
                BX.create('div', {
                    ...
                    html: '<a class="plImg" href="' + item.DETAIL_PAGE_URL + '"><img src="' + (!!item.PREVIEW_PICTURE_SRC ? item.PREVIEW_PICTURE_SRC : this.templateFolder + '/images/empty_cart.svg') + '" alt="' + item.NAME + '"></a>'
                })
            );
            itemNodeInnerInner.appendChild(
                BX.create('div', {
                    ...
                    html: '<a class="plName" href="' + item.DETAIL_PAGE_URL + '">' + item.NAME + '</a><div class="prodLink__q">' + item.QUANTITY + ' ' + item.MEASURE_TEXT + '</div><div class="prodLink__values"><div class="prodLink__new-value">' + item.SUM + '</div><div class="prodLink__old-value">' + (item.SUM !== item.SUM_BASE_FORMATED ? item.SUM_BASE_FORMATED : '') + '</div></div>'
                })
            );
        ...
        for (let l in this.result.ORDER_PROP.properties) {
            ...
            if(item['IS_LOCATION'] === 'Y') {
                ...
                let cityChanger = BX.create('div', {
                    ...
                    html: '<input class="customInput" type="text" data-location><span class="locationFullName"></span><input type="hidden" name="ORDER_PROP_' + item.ID + '" id="soa-property-' + item.ID + '" value="' + item.VALUE + '"><div class="cityChanger__dd"><div class="cityDropDown"><ul class="cityDropDown__list"></ul></div></div>'
                })
    ...
    commentInit: function () {
        ...
        nodePolicy.appendChild(
            BX.create('div', {
                ...
                html: '<div class="ps-text__inner"><div class="ps-text__inf">Нажимая кнопку «Подтвердить заказ», я принимаю условия <a href="#">политики конфиденциальности</a> и <a href="#">пользовательского соглашения</a></div>'
            })
        );
